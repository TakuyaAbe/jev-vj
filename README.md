...
最下行は `jev@sayu_nomu_:~$ ●○○○ bar 42 · 128bpm ▮▮▮▯▯ · tunnel int0.61 cold · build · armed→strobe · MAGI… █` のプロンプト 1 行に集約（旧 HUD は削除）。20 秒より古い行は徐々に薄くなる。
...
1 呼び出しの入力は約 4k tokens（以前は 5.5k）。内訳は「質問の選択肢」が大半なので、そこを削っている:

- シーンの選択肢は `SCENE_SHORT`（`src/jev.ts`、1 つ 30 字前後）を使う。人が読む `description` はパネル用で、Jev には送らない。
- 先読み質問（`drop_scene`, `kime_on_drop`）は「ドロップが来そうなとき」だけ聞く（直前の合議が drop / steady で drop_soon < 0.3、armed なし、定期審議 → 省略。ログに `先読み省略`）。
- `recent_bars` は 8 小節、各質問の文言も短縮。
- **MAGI の使い方**（パネルの「Context for Jev」欄）: `常時 3 体` / `変化時のみ 3 体（定期は 1 体）` / `1 体だけ`。3 体は 1 審議 = 3 リクエストなので、「変化時のみ」にすると定期審議が 1/3 になる。
- **候補を絞る**（Scenes 欄のプリセット / チェック）: 選択肢が 17 → 5 になれば 1 呼び出しあたりさらに約 700 tokens 減る。

目安（1 審議あたり）: 常時 3 体・全質問 ≈ 12k、変化時のみ 3 体 ≈ 4〜12k、1 体＋先読み省略＋候補 5 個 ≈ 2.5k。
